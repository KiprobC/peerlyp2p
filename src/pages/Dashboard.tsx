...
const Dashboard = () => {
  ...
  const DesktopSidebar = () => (
    <aside className={`fixed inset-y-0 left-0 z-40 hidden border-r border-border/70 bg-card/95 backdrop-blur-xl md:flex md:flex-col transition-[width] duration-200 ${sidebarCollapsed ? "w-20" : "w-64"}`}>
      <div className="flex h-20 items-center border-b border-border/60 px-5"><Link to="/dashboard" className="flex min-w-0 items-center gap-2"><img src={sidebarCollapsed ? peerlyIcon : peerlyLogo} alt="Peerly" className={sidebarCollapsed ? "h-9 w-9 rounded-xl" : "h-8 w-auto max-w-[145px] object-contain object-left"} />{!sidebarCollapsed && <span className="sr-only">Peerly</span>}</Link></div>
